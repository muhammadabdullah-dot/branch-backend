"""The fixed asset register, its schedules, the monthly depreciation run and disposals.
...
async def ensure_accounts() -> tuple[Account, Account]:
    ...
    Both are made or marked once; head office hears about them like any other change to the chart."""
    depreciation = await Account.get_or_none(system_key=DEPRECIATION_KEY)
    if depreciation is None:
        await accounts_chart_service.ensure_standard_chart()
        depreciation = (await Account.filter(system_key=None, code="52010008", name__iexact="DEPRECIATION").first()
                        or await Account.filter(system_key=None, group_id=EXPENSE_GROUP, name__iexact="DEPRECIATION").first())
        if depreciation is None:
            depreciation = await Account.create(
                code=await accounts_chart_service.next_account_code(EXPENSE_GROUP), name="DEPRECIATION", group_id=EXPENSE_GROUP,
                sub_group_id=f"{EXPENSE_GROUP}01", system_key=DEPRECIATION_KEY, standard=True,
            )
        else:
            depreciation.system_key = DEPRECIATION_KEY
            await depreciation.save(update_fields=["system_key", "updated_at"])
        await accounts_chart_service.emit_account(depreciation)
    disposal = await Account.get_or_none(system_key=DISPOSAL_KEY)
    if disposal is None:
        disposal = await Account.filter(system_key=None, group_id=OTHER_INCOME_GROUP, name__iexact=DISPOSAL_NAME).first()
        if disposal is None:
            disposal = await Account.create(
                code=await accounts_chart_service.next_account_code(OTHER_INCOME_GROUP), name=DISPOSAL_NAME, group_id=OTHER_INCOME_GROUP,
                sub_group_id=f"{OTHER_INCOME_GROUP}01", system_key=DISPOSAL_KEY, standard=True,
            )
        else:
            disposal.system_key = DISPOSAL_KEY
            await disposal.save(update_fields=["system_key", "updated_at"])
        await accounts_chart_service.emit_account(disposal)
    return depreciation, disposal


async def _group_types() -> dict[str, str]:
    """Group code -> account type (1 assets … 5 expenses)."""
    types = {c.code: c.type_id for c in await AccountCategory.all()}
    return {g.code: types.get(g.category_id, "") for g in await AccountGroup.all()}
...
async def pick_lists(access: accounts_areas.Access) -> dict:
    ...
    rows = await Account.filter(active=True).order_by("code")

    def seen(a: Account) -> bool:
        return access.can_see(*areas.get(str(a.id), (None, None)))

    fixed = [a for a in rows if seen(a) and areas[str(a.id)][0] == "fixed-assets" and types.get(_group_code(a)) == "1"]
    return {
        ...
        "expense": [_ref(a) for a in sorted((a for a in rows if seen(a) and types.get(_group_code(a)) == "5"),
...
async def refresh() -> None:
    ...
    for run in await DepreciationRun.filter(status__in=["draft", "posted"]).prefetch_related("voucher"):
        state = voucher_state(run.voucher)
        if state != run.status:
            run.status = state
            await run.save(update_fields=["status", "updated_at"])
    for asset in await FixedAsset.filter(disposal_date__isnull=False).prefetch_related("disposal_voucher"):
...
async def register(access: accounts_areas.Access) -> dict:
    ...
    everything = await _loaded(FixedAsset.all().order_by("code"))
    ...
    all_accounts = await Account.all()
    fixed_ids = {str(a.id) for a in all_accounts if a.group_id == FIXED_GROUP} | {str(a.category_account_id) for a in everything}
    accumulated_ids = {str(a.id) for a in all_accounts if a.group_id == ACCUMULATED_GROUP} | {str(a.accumulated_account_id) for a in everything}
...
async def runs_out(access: accounts_areas.Access, limit: int = 36) -> list[dict]:
    if not access.can_see("fixed-assets"):
        return []
    runs = await DepreciationRun.all().order_by("-month", "-created_at").limit(limit).prefetch_related("voucher")
    counts = {}
...
async def get_asset(asset_id: str, access: accounts_areas.Access) -> FixedAsset:
    try:
        asset = await _loaded(FixedAsset.filter(id=asset_id))
    except (ValueError, TypeError):
        asset = []
    if not asset:
        raise AssetError("That asset isn't on the register.")
    area, key = (await accounts_areas.areas_by_account()).get(str(asset[0].category_account_id), (None, None))
...
async def _find(account_id) -> Account | None:
    try:
        return await Account.get_or_none(id=account_id) if account_id else None
    except (ValueError, TypeError):
        return None
...
async def _check_accounts(access: accounts_areas.Access, category: Account, accumulated: Account, expense: Account) -> None:
    types = await _group_types()
    areas = await accounts_areas.areas_by_account()
    if areas.get(str(category.id), (None,))[0] != "fixed-assets" or types.get(category.group_id) != "1" or category.group_id == ACCUMULATED_GROUP:
        raise AssetError(f"{category.name} isn't a fixed asset account. Pick one under FIXED ASSETS.")
    if accumulated.group_id != ACCUMULATED_GROUP:
...
async def _fields(payload: dict, access: accounts_areas.Access, current: FixedAsset | None = None) -> dict:
    ...
    if not accumulated_id:
        first = await Account.filter(group_id=ACCUMULATED_GROUP, active=True).order_by("code").first()
...
async def create(user: User, payload: dict, access: accounts_areas.Access) -> FixedAsset:
    ...
    code = f"FA-{seq:04d}"
    while await FixedAsset.exists(code=code):
        seq = await next_value("fixed-asset", 1)
        code = f"FA-{seq:04d}"
    return await FixedAsset.create(code=code, created_by_name=user.name, **fields)
...
async def delete(asset_id: str, access: accounts_areas.Access) -> None:
    ...
    area, key = (await accounts_areas.areas_by_account()).get(str(asset.category_account_id), (None, None))
    if not access.can_use(area, key):
...
async def run_preview(month_value, access: accounts_areas.Access) -> dict:
    ...
    existing = await DepreciationRun.filter(month=month, status__in=["draft", "posted"]).prefetch_related("voucher").first()
    if existing:
    ...
    areas = await accounts_areas.areas_by_account()
    lines, hidden = [], 0
    for asset in await _loaded(FixedAsset.filter(status="in_use", depreciation_start__lte=month, purchase_date__lte=end).order_by("code")):
