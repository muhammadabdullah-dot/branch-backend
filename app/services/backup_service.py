"""Backup and restore: a restorable copy of this server's whole database (and its pictures), in a folder someone chose.
...
Restore puts a backup back over the live database. It is deliberately hard to do by accident: only a Branch Manager can,
the word RESTORE has to be typed, the backup must belong to this branch and pass its checksum and SQLite's own integrity
...
COUNT_TABLES = {"sales": "sale_records", "items": "products", "customers": "parties", "staff": "users", "vouchers": "acc_vouchers", "grns": "grns"}
...
def _identity_from(conn: sqlite3.Connection, tables: set[str]) -> dict:
    if "branch_identity" in tables:
        row = conn.execute("SELECT code, name FROM branch_identity LIMIT 1").fetchone()
        if row:
...
def _make_backup(kind: str, folder: Path, created_by: str | None, note: str | None, include_media: bool) -> dict:
    ...
    code = (info_live.get("code") or APP_LABEL.upper()).replace(" ", "")
...
async def save_upload(stream, filename: str, uploaded_by: str | None) -> dict:
    ...
    try:
        ...
        code = (manifest.get("branchCode") or APP_LABEL.upper()).replace(" ", "")
...
async def _pause_background() -> None:
    from app.core import accounts_scheduler, scheduler
