"""The legacy Purchase Summary Group Wise report: what was bought and sent back, grouped by brand (or any other way
...
`approved`: "all" (default), "yes" or "no" narrows the deliveries by their approval. Returns aren't approved
separately, so they're always counted.
"""
...
from app.models import GRN, GRNLine, Product, PurchaseReturn, PurchaseReturnLine
...
async def summary(group_by: str, view: str, from_at: datetime | None, to_at: datetime | None,
    ...
    grns = in_window(GRN.all(), "at", from_at, to_at)
    if approved == "yes":
        grns = grns.filter(approved=True)
    elif approved == "no":
        grns = grns.filter(approved=False)
    returns = in_window(PurchaseReturn.all(), "at", from_at, to_at)
    if supplier_id:
    ...
    return_ids = await returns.values_list("id", flat=True)

    purchase_lines = [
        {**l, "supplier": l["grn__supplier__name"]}
        for l in await GRNLine.filter(grn_id__in=list(grn_ids)).values(
            "grn_id", "grn__supplier__name", "product_id", "qty", "bonus_qty", "unit_price", "disc_percent", "flat_disc", "misc", "tax_rate",
        )
    ] if grn_ids else []
    return_lines = [
        {**l, "return_id": l["purchase_return_id"], "supplier": l["purchase_return__supplier__name"]}
        for l in await PurchaseReturnLine.filter(purchase_return_id__in=list(return_ids)).values(
            "purchase_return_id", "purchase_return__supplier__name", "product_id", "qty", "unit_price", "tax_rate",
        )
    ] if return_ids else []

    product_ids = {str(l["product_id"]) for l in purchase_lines} | {str(l["product_id"]) for l in return_lines}
    ...
    result = fold(group_by, view, purchase_lines, return_lines, products)
