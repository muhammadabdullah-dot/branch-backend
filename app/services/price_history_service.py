"""Price history: every change to what an Item sells or costs for, one row per value that moved.
...
The Item form's Price history tab, the Price Changes report and Labels ("Add changed Items") all read these
rows. Rows written before 18 Sep 2026 hold the sale and retail price together with no `field`; `entries`
reads them as the sale price change they were, plus the retail price when that moved too.
"""
...
SOURCES: dict[str, str] = {
    ...
    "transfer": "Received on a shipment",
...
def first_price(product: Product, source: str, reference: str | None = None, user: User | None = None) -> ProductPriceChange:
    """A new Item's starting sale price: the first line of its history, and a shelf tag to print."""
...
def source_label(source: str, reference: str | None) -> str:
    ...
    return {
        ...
        "transfer": f"Received on shipment {ref}",
        "transfer-new": f"Added by head office shipment {ref}",
        "return": f"Customer return {ref}",
