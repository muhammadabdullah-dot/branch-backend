...
async def _groups() -> dict[str, str]:
    return {g.code: g.category_id for g in await AccountGroup.all()}


async def account_area(account: Account, groups: dict[str, str] | None = None) -> str | None:
    groups = groups if groups is not None else await _groups()
    return area_of(account.kind, account.group_id, groups.get(account.group_id), account.system_key)


async def areas_by_account() -> dict[str, tuple[str | None, str | None]]:
    """Every account's area and system key, by account id."""
    groups = await _groups()
    rows = await Account.all().values("id", "kind", "group_id", "system_key")
    return {str(r["id"]): (area_of(r["kind"], r["group_id"], groups.get(r["group_id"]), r["system_key"]), r["system_key"]) for r in rows}
...
async def check_group_change(access: Access, *, category_code: str | None = None, group_code: str | None = None,
    ...
    if access.sees_everything:
        return
    if sub_group_code:
        from app.models import AccountSubGroup

        sub = await AccountSubGroup.get_or_none(code=sub_group_code)
        group_code = sub.group_id if sub else group_code
    groups = await _groups()
    area = area_of(None, group_code, groups.get(group_code or "") or category_code)
...
async def check_account_change(access: Access, account: Account | None, *, group_code: str | None = None, kind: str | None = None) -> None:
    ...
    if group_code:
        ...
        area = area_of(kind, group_code, groups.get(group_code))
