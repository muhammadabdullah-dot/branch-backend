"""Tax reports over HTTP: GST month by month with the vouchers behind it, the set-off voucher, and withholding, advance
and income tax. See services/tax_reports_service.py."""
...
from app.services import accounts_areas, accounts_posting_service, tax_reports_service, vouchers_service
...
@router.get("/gst")
async def gst(from_: date | None = Query(None, alias="from"), to: date | None = None, user: User = Depends(_reader)) -> dict:
    await accounts_posting_service.ensure_recent()
    start, end = _range(from_, to)
    return await tax_reports_service.gst(start, end)


@router.get("/withholding")
async def withholding(from_: date | None = Query(None, alias="from"), to: date | None = None, user: User = Depends(_reader)) -> dict:
    await accounts_posting_service.ensure_recent()
    start, end = _range(from_, to)
    return await tax_reports_service.withholding(start, end)
...
async def prepare_set_off(payload: SetOffIn, user: User = Depends(_reader)) -> dict:
    """A draft journal: Dr GST payable, Cr GST input. Writing it is writing a voucher, on accounts in the Tax area."""
